...
//=============================================================
bool AudioFileWav::load (std::string filePath) {
  ...
  if (audioFileFormat == AudioFileFormat::Wave) {
    if(verbose)std::cout << "Audio File Type: " << "Wave" << std::endl;
    return decodeWaveFile (fileData);
  }
  else if (audioFileFormat == AudioFileFormat::Aiff) {
    if(verbose)std::cout << "Audio File Type: " << "Aiff" << std::endl;
...
//=============================================================
bool AudioFileWav::decodeWaveFile (std::vector<uint8_t>& fileData) {
  // -----------------------------------------------------------
  // HEADER CHUNK
  if(verbose)std::cout << "\n############## HEADER CHUNK ##############" << '\n';
  int f = 0;
  std::string headerChunkID (fileData.begin()+f, fileData.begin() +f+4); f+=4;
  int32_t fileSizeInBytes = fourBytesToInt (fileData, f) + 8; f+=4;
  std::string format (fileData.begin() + f, fileData.begin() + +f+4); f+=4;
  if(verbose)std::cout << "headerChunkID:     " << headerChunkID << '\n';
  if(verbose)std::cout << "fileSizeInBytes:   " << fileSizeInBytes << '\n';
  if(verbose)std::cout << "format:            " << format << '\n';
  ...
  std::string formatChunkID (fileData.begin() + f, fileData.begin() + f+4); f+=4;
  int32_t formatChunkSize = fourBytesToInt (fileData, f); f+=4;
  int16_t audioFormat = twoBytesToInt (fileData, f); f+=2;
  int16_t numChannels = twoBytesToInt (fileData, f); f+=2;
  sampleRate = (uint32_t) fourBytesToInt (fileData, f); f+=4;
  int32_t numBytesPerSecond = fourBytesToInt (fileData, f); f+=4;
  int16_t numBytesPerBlock = twoBytesToInt (fileData, f); f+=2;
  bitDepth = (int) twoBytesToInt (fileData, f); f+=2;

  int numBytesPerSample = bitDepth / 8;

  if(verbose)std::cout << "formatChunkID:     " << formatChunkID << '\n';
  if(verbose)std::cout << "formatChunkSize:   " << formatChunkSize << '\n';
  if(verbose)std::cout << "audioFormat:       " << audioFormat << '\n';
  if(verbose)std::cout << "numChannels:       " << numChannels << '\n';
  if(verbose)std::cout << "sampleRate:        " << sampleRate << '\n';
  if(verbose)std::cout << "numBytesPerSecond: " << numBytesPerSecond << '\n';
  if(verbose)std::cout << "numBytesPerBlock:  " << numBytesPerBlock << '\n';
  if(verbose)std::cout << "bitDepth:          " << bitDepth << '\n';
  if(verbose)std::cout << "numBytesPerSample: " << numBytesPerSample << '\n';
  ...
  if(verbose)std::cout << "\n############### DATA CHUNK ###############" << '\n';
  f = indexOfDataChunk;
  std::string dataChunkID (fileData.begin() + f, fileData.begin() + f + 4);f+=4;
  int32_t dataChunkSize = fourBytesToInt (fileData, f);f+=4;

  int numSamples = dataChunkSize / (numChannels * bitDepth / 8);
  int samplesStartIndex = f;

  if(verbose)std::cout << "dataChunkID:       " << dataChunkID << '\n';
  if(verbose)std::cout << "dataChunkSize:     " << dataChunkSize << '\n';
  if(verbose)std::cout << "numSamples:        " << numSamples << '\n';
  if(verbose)std::cout << "samplesStartIndex: " << samplesStartIndex << '\n';
  ...
  for (int i = 0; i < numSamples; i++) {
    for (int channel = 0; channel < numChannels; channel++) {
      ...
      if (bitDepth == 8) {
        double sample = singleByteToSample (fileData[sampleIndex]);
...
//=============================================================
bool AudioFileWav::decodeAiffFile (std::vector<uint8_t>& fileData) {
  // -----------------------------------------------------------
  // HEADER CHUNK
  ...
  for (int i = 0; i < numSamplesPerChannel; i++) {
    for (int channel = 0; channel < numChannels; channel++) {
      ...
      if (bitDepth == 8) {
        ...
        double sample = (double)sampleAsSigned8Bit / (double)128.;
        samples[channel].push_back (sample);
      }
      else if (bitDepth == 16) {
...
//=============================================================
uint32_t AudioFileWav::getAiffSampleRate (std::vector<uint8_t>& fileData, int sampleRateStartIndex) {
...
//=============================================================
bool AudioFileWav::tenByteMatch (std::vector<uint8_t>& v1, int startIndex1, std::vector<uint8_t>& v2, int startIndex2) {
  for (int i = 0; i < 10; i++) {
...
//=============================================================
void AudioFileWav::addSampleRateToAiffData (std::vector<uint8_t>& fileData, uint32_t sampleRate) {
...
//=============================================================
bool AudioFileWav::save (std::string filePath, AudioFileFormat format) {
...
//=============================================================
bool AudioFileWav::saveToWaveFile (std::string filePath) {
  ...
  for (int i = 0; i < getNumSamplesPerChannel(); i++) {
    for (int channel = 0; channel < getNumChannels(); channel++) {
      if (bitDepth == 8) {
        ...
        int32_t sampleAsIntAgain = (int32_t) (samples[channel][i] * (double)8388608.);
...
//=============================================================
bool AudioFileWav::saveToAiffFile (std::string filePath) {
  std::vector<uint8_t> fileData;
...
//=============================================================
bool AudioFileWav::writeDataToFile (std::vector<uint8_t>& fileData, std::string filePath) {
...
//=============================================================
void AudioFileWav::addStringToFileData (std::vector<uint8_t>& fileData, std::string s) {
  for (int i = 0; i < s.length();i++)
    fileData.push_back ((uint8_t) s[i]);
}

//=============================================================
void AudioFileWav::addInt32ToFileData (std::vector<uint8_t>& fileData, int32_t i, Endianness endianness) {
  uint8_t bytes[4];
...
//=============================================================
void AudioFileWav::addInt16ToFileData (std::vector<uint8_t>& fileData, int16_t i, Endianness endianness) {
  uint8_t bytes[2];
...
//=============================================================
int32_t AudioFileWav::fourBytesToInt (std::vector<uint8_t>& source, int startIndex, Endianness endianness) {
  int32_t result;
...
//=============================================================
int16_t AudioFileWav::twoBytesToInt (std::vector<uint8_t>& source, int startIndex, Endianness endianness) {
  int16_t result;
...
//=============================================================
int AudioFileWav::getIndexOfString (std::vector<uint8_t>& source, std::string stringToSearchFor) {
  int index = -1;
...
//=============================================================
double AudioFileWav::sixteenBitIntToSample (int16_t sample) {
  return static_cast<double> (sample) / static_cast<double> (32768.);
}

//=============================================================
int16_t AudioFileWav::sampleToSixteenBitInt (double sample) {
  sample = clamp (sample, -1., 1.);
  return static_cast<int16_t> (sample * 32767.);
}

//=============================================================
uint8_t AudioFileWav::sampleToSingleByte (double sample) {
  sample = clamp (sample, -1., 1.);
  sample = (sample + 1.) / 2.;
...
//=============================================================
double AudioFileWav::singleByteToSample (uint8_t sample) {
  return static_cast<double> (sample - 128) / static_cast<double> (128.);
}

//=============================================================
double AudioFileWav::clamp (double value, double minValue, double maxValue) {
...
//=============================================================
AudioFileFormat AudioFileWav::determineAudioFileFormat (std::vector<uint8_t>& fileData) {
